...
enum root_tie_breaker_type { NONE, SUM, MIN };
...
public:
	...
	root_tie_breaker_type root_tb_type;
	...
	CBSHeuristic(int num_of_agents,
               ...
               MDDTable& mdd_helper, const vector<vector<MeetingsTable*>>& meeting_tables) : num_of_agents(num_of_agents),
                                       paths(paths), search_engines(search_engines),
                                       initial_constraints(initial_constraints), mdd_helper(mdd_helper), m_meeting_tables(meeting_tables) {}

	void init();
	int calcClearance(int meeting_location, int task_id, vector<Task>& tasks) const;
	int computeMeetingsTieBreaker(CBSNode& node);
...
private:
	...
	vector<std::map<int, int>> clearance_cache; // the vector index is for task_idx and the map is <meeting_location,clearance>
	const vector<vector<MeetingsTable*>>& m_meeting_tables;
