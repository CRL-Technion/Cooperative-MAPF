...
enum search_assignments { NOTA, GREEDY, SINGLE, FULL }; // whether to search over assignments
...
public:
	...
	double runtime_generate_root = 0; // runtime of generating root nodes
	...
	double runtime_cost_matrix = 0.0; // runtime for computing the global cooperation cost matrix
	double runtime_assignments = 0.0; // runtime for performing task assignment
	...
	uint64_t num_HL_expanded = 0;  // how many total nodes were extracted from open (focal) and expanded
	uint64_t num_HL_generated = 0; // how many total nodes were generated and inserted to open
	uint64_t num_LL_expanded = 0;
	uint64_t num_LL_generated = 0;

	// Co-CBS
	uint64_t num_root_expanded = 0; // how many root nodes were extracted from open (focal) and expanded
	uint64_t num_root_generated = 0; // how many root nodes were generated and inserted to open
	...
	void setRootTbType(root_tie_breaker_type type) {heuristic_helper.root_tb_type = type; }
	...
	void setProot(double p) { if (p >=0.0 && p <= 1.0) m_p_root = p; }
	void setLazyExpansion(double l) { lazy_expansion = l; }
	void setTaskAssignment(search_assignments sa) { ta_type = sa; }
	void setNumAssignments(int nm) { num_assignments = nm; }
	void setRunBaseline(bool bl) { m_run_baseline = bl; }

	////////////////////////////////////////////////////////////////////////////////////////////
	// Runs the algorithm until the problem is solved or time is exhausted 
	bool solve(double time_limit, int cost_lowerbound = 0, int cost_upperbound = MAX_COST);

	CoCBS(const Instance& instance, bool sipp, int screen);
	CoCBS(vector<SingleAgentSolver*>& search_engines,
...
private:
	...
	bool lazy_expansion; // lazy expansion of root nodes
	...
	pairing_heap< CBSNode*, compare<CBSNode::secondary_compare_node> > root_focal_list;
	...
	CostType m_cost_type = SOC; // TODO Nir currently supporting only SOC
	...
	vector<Path*> paths;
	vector<vector<Path>> paths_found_initially;  // contain initial paths found
	...
	bool generateRootNode(CBSNode* new_root, CBSNode* curr_root, int task_dx);
	bool generateRoot();
	...
	bool validateSolution(CBSNode& node) const;
	inline int getAgentLocation(int agent_id, size_t timestep) const;
	inline void pushNode(CBSNode* node);

	// Co-CBS
	void computeMeetingTables(int assignment_idx);
	Dijkstra* dijkstra;
	vector<vector<MeetingsTable*>> m_meeting_tables;
	std::set<std::vector<int>> m_meetings_record;
	double m_p_root = 0.0;
	vector<AssignmentSet> m_assignments;
	vector<double> assignment_times;

	friend class MeetingsTable;
	void planPaths();
	void expandRoot(CBSNode * node);
	void printFocalList() const;
	void printOpenList() const;
	CBSNode* extractNode();
	bool planRootNode(CBSNode* node);
	bool planAssignmentNode(CBSNode* node);
	void task_assignment();
	Assignment getAssignedAgents(int assignment_idx, int task_idx);
	int getAgentTask(int assignment_idx, int agent_idx) const;
	coder::array<double, 3U> computeGlobalCostMatrix() const;
	bool m_run_baseline = false;
	bool solve_baseline(CBSNode* curr);
};
