...
public:
	...
	virtual Path findPath(const CBSNode& node, const ConstraintTable& initial_constraints,
		const vector<Path*>& paths, int agent, int lower_bound, int goal_time=-1, int start_time=0) = 0;
	...
	SingleAgentSolver(const Instance& instance, int start, int goal) :
		instance(instance), //agent(agent), 
	...
	void setGoalLocation(int new_goal);
	void setStartLocation(int new_start) { start_location = new_start; }
