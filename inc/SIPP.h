...
public:
	...
	struct NodeHasher
	{
		std::size_t operator()(const SIPPNode* n) const
		{
			...
			size_t timestep_hash = std::hash<int>()(std::get<0>(n->interval));
		...
		bool operator()(const SIPPNode* n1, const SIPPNode* n2) const
		{
			return (n1 == n2) ||
				   (n1 && n2 && n1->location == n2->location &&
					...
					std::get<0>(n1->interval) == std::get<0>(n2->interval)); //TODO: do we need to compare timestep here?
		}
	};
};
...
public:
	...
	Path findPath(const CBSNode& node, const ConstraintTable& initial_constraints,
		const vector<Path*>& paths, int agent, int lowerbound, int goal_time=-1, int start_time=0);
