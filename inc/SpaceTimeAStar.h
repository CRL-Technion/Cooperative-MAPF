...
public:
	...
	Path findPath(const CBSNode& node, const ConstraintTable& initial_constraints,
				  const vector<Path*>& paths, int agent, int lower_bound, int goal_time=-1, int start_time=0);
	...
	SpaceTimeAStar(const Instance& instance, int start, int goal) :
			SingleAgentSolver(instance, start, goal) {}
...
private:
	...
	bool isGoalReached(AStarNode* node, int holding_time, int goal_time);
