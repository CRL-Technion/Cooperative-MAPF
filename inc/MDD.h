...
public:
	...
	bool valid, valid_back = false; // whether this node is on a valid path to goal (e.g., it passes through a certain node)
...
public:
	...
	bool buildMDD(const ConstraintTable& ct,
				  int num_of_levels, SingleAgentSolver* solver, int agent, Task task, Meeting m);
	...
	void clear();
	bool isMeetingSatisfied(Meeting m, int loc, int t);
...
public:
	...
	MDD* getMDD(CBSNode& node, int agent, size_t mdd_levels, Task* task = nullptr);
	void findSingletons(CBSNode& node, int agent, Path& path, Task* task);
