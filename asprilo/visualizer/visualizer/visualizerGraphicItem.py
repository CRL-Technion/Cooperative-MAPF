...
class RobotType(Enum):  # TODO import it
    fetch = 0
    drive = 1
...
class Shelf(VisualizerGraphicItem):
    ...
    def __init__(self, ID = 0, x = 0, y = 0):
        ...
        self._products   = []
        self._graphics_item = QGraphicsRectItem(self)
        self._graphics_carried = QGraphicsRectItem()
    ...
    def set_picked(self, picked):
        self._picked = picked
        if picked:
            self.remove_product(self.get_id(), 1)
        else:
            self.remove_product(self.get_id(), -1)
    ...
    def determine_color(self, number, count, pattern = None):
        ...
        """
        if self.get_product_amount(self.get_id()) > 0:
            color = QColor(255.0, 255.0, 0.0)
        else:
...
class Robot(VisualizerGraphicItem):
    ...
    def __init__(self, ID = 0, x = 0, y = 0):
        ...
        self._picked_graphics_item = QGraphicsRectItem(self)
    ...
    def set_starting_position(self, x, y):
        ...
        super().set_starting_position(x, y)
    ...
    def set_rect(self, rect):
        ...
        if self._display_mode == 0:
            if bold:
                self._text.setHtml(f'<b>{self._initial}(' + str(self._id) + ')</b>')
            else:
                self._text.setHtml(f'{self._initial}(' + str(self._id) + ')')
            if not self._highlighted:
                ...
                rect2 = QRectF(rect.x() + 0.05*rect.width(), 
                               ...
                               rect.height()*0.9,)
            if self._picks:
                item_rect = QRectF(rect.x() + 0.4 * rect.width(),
                                   rect.y() + 0.4 * rect.height(),
                                   rect.width() * 0.2,
                                   rect.height() * 0.2)
                self._picked_graphics_item.setBrush(QBrush(QColor(255.0, 255.0, 0.0)))
            ...
            if self._picks:
                item_rect = QRectF(rect.x() + 0.3 * rect.width(),
                                   rect.y() + 0.3 * rect.height(),
                                   rect.width() * 0.4,
                                   rect.height() * 0.4)
                self._picked_graphics_item.setBrush(QBrush(QColor(255.0, 255.0, 0.0)))

        self._picked_graphics_item.setRect(item_rect)
    ...
    def parse_init_value(self, name, value):
        ...
        result = super().parse_init_value(name, value)
    ...
    def do_action(self, time_step):
        ...
        if time_step >= self.get_lifetime():
            # hide robots that finished their actions
        ...
        if action.name == 'move':
            ...
            # check for delivery or meeting
            shelf = self._model.filter_items(item_kind='shelf',
                                             position=self._position,
                                             return_first=True)[0]
            if shelf is not None:
                assert self.get_kind_name() == 'fetch'
                shelf.set_picked(True)
                self.set_picks(True)

            if time_step == (self.get_lifetime() - 1):
                if self.get_kind_name() == 'fetch':
                    self.set_picks(False)
                    # find the corresponding drive unit and transfer the item
                    drive = self._model.filter_items(item_kind='drive',
                                                     ID=self.get_id(),
                                                     return_first=True)[0]
                    drive.set_picks(True)
                else:
                    self.set_picks(False)
    ...
    def undo_action(self, time_step):
        ...
        if action.name == 'move':
            ...
            # TODO need to handle item pickups in undo
...
class FetchUnit(Robot):
    def __init__(self, ID=0, x=0, y=0):
        ...
        super().__init__(ID, x, y)
        self._kind_name = RobotType.fetch.name
        self._initial = 'F'
    ...
    def determine_color(self, number, count, pattern = None):
        ...
        color = calculate_color(QColor(255.0, 0.0, 0.0), QColor(255.0, 0.0, 0.0), (float)(number)/count)

        if self._disappeared:
            color = QColor(color.red(), color.green(), color.blue(), 10)
...
class DriveUnit(Robot):
    def __init__(self, ID=0, x=0, y=0):
        ...
        super().__init__(ID, x, y)
        self._kind_name = RobotType.drive.name
        self._initial = 'D'
    ...
    def determine_color(self, number, count, pattern = None):
        ...
        color = calculate_color(QColor(0.0, 0.0, 255.0), QColor(0.0, 0.0, 255.0), (float)(number)/count)

        if self._disappeared:
            color = QColor(color.red(), color.green(), color.blue(), 10)

        # for drive units, in the meeting, set their color to purple
        # find the corresponding fetch unit and check if they meet
        fetch = self._model.filter_items(item_kind='fetch',
                                         ID=self.get_id(),
                                         position=self.get_position(),
                                         return_first=True)[0]
        if fetch is not None:
            color = QColor(125.0, 0.0, 125.0)
