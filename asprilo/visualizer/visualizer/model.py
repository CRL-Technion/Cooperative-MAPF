...
class Model(object):
    ...
    def create_item(self, item_kind, ID = None, add_immediately = False):
        ...
        if ID is None:
            ...
            elif item_kind == 'fetch':
                if 'fetch' not in self._graphic_items:
                    self._graphic_items['fetch'] = {}
                dic = self._graphic_items['fetch']
            elif item_kind == 'drive':
                if 'drive' not in self._graphic_items:
                    self._graphic_items['drive'] = {}
                dic = self._graphic_items['drive']
    ...
    def update(self, update_windows = True):
        if self._current_step > self._num_steps + 1 or self._num_steps == 0:
    ...
    def skip_to_end(self):
        if self._editable:
            return
        while(self.update(False) <= self._num_steps + 1):
            pass
        self.update_windows()
