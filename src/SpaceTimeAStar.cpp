...
Path SpaceTimeAStar::findPath(const CBSNode& node, const ConstraintTable& initial_constraints,
							  const vector<Path*>& paths, int agent, int lowerbound, int goal_time, int start_time)
{
	// node.printConstraints(agent); std::cout << std::endl;
	num_expanded = 0;
	num_generated = 0;
	...
	if (constraint_table.length_min >= MAX_TIMESTEP ||
		...
		constraint_table.constrained(start_location, start_time)) // the agent cannot stay at its start location
	{
		return Path();
	}
	...
	{
		// TODO Nir: haven't dealt with this section yet
		...
		{
			if (goal_time == -1)
				return findShortestPath(constraint_table, make_pair(start_location, start_time), lowerbound);
			else
				return findShortestPath(constraint_table, make_pair(start_location, start_time), lowerbound, std::make_pair(goal_location, goal_time));
		}
...
{
	if (start_state.second > landmark.second) // goal time is eariler than start time
		return Path();
...
bool SpaceTimeAStar::isGoalReached(AStarNode* node, int holding_time, int goal_time)
{
	if (goal_time == -1) // no goal time
		return (node->location == goal_location && // arrive at the goal location
				!node->wait_at_goal && // not wait at the goal location
				node->timestep >= holding_time); // the agent can hold the goal location afterward
	return (node->location == goal_location && // arrive at the goal location
				node->timestep == goal_time && // arrive at the goal time
				!node->wait_at_goal && // not wait at the goal location
				node->timestep >= holding_time); // the agent can hold the goal location afterward
}