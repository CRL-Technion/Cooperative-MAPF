...
Instance::Instance(const string& map_fname, const string& agent_fname, 
	int num_of_agents, int num_of_tasks, const string& agent_indices, 
	int num_of_rows, int num_of_cols, int num_of_obstacles, int warehouse_width):
	map_fname(map_fname), agent_fname(agent_fname), num_of_agents(num_of_agents), num_of_tasks(num_of_tasks), agent_indices(agent_indices)
{
	bool succ = loadMap();
	if (!succ)
	{
	...
	// Construct the tasks vector
	tasks.resize(num_of_tasks);
	for (int i = 0; i < num_of_tasks; ++i)
	{
		Task t(task_start_locations[i], task_goal_locations[i], i*2, start_locations[i*2], i*2+1, start_locations[i*2+1]);
		tasks[i] = t;
	}
...
bool Instance::loadAgents()
{
	...
	if (line[0] == 'v') // Nathan's benchmark
	{
		...
		task_start_locations.resize(num_of_tasks);
	    task_goal_locations.resize(num_of_tasks);
		...
		while(i < num_of_agents)
		{
			...
			if (count == ids[i])
			{
				...
				if (task_line)
				{
					goal_locations[i/2] = linearizeCoordinate(row, col);
					task_start_locations[i/2] = linearizeCoordinate(row, col);
				}
				else
					start_locations[i-1] = linearizeCoordinate(row, col);
				...
				if (task_line)
					task_goal_locations[i/2] = linearizeCoordinate(row, col);
				else
