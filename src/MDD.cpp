...
bool MDD::buildMDD(const ConstraintTable& ct, int num_of_levels, SingleAgentSolver* _solver, int agent, Task task, Meeting m)
{
	this->solver = _solver;
	// set start and goal locations in solver by agent type
	if (agent % 2 == 0) // TODO Nir this should be more generic.
	{
		_solver->setStartLocation(std::get<3>(task)); // agent A start
		_solver->setGoalLocation(std::get<0>(m)); // meeting location
	} else {
		_solver->setStartLocation(std::get<5>(task)); // agent B start
		_solver->setGoalLocation(std::get<1>(task)); // task goal
	}
	
  	auto root = new MDDNode(solver->start_location, nullptr); // Root
	if (root->location == std::get<0>(task))
		root->valid = true;
	...
	while (!open.empty())
	{
		...
		int heuristicBound = num_of_levels - curr->level - 2; // TODO Nir take intermediate node (task start for A or meeting for B) for more informed heuristicBound
		...
		for (int next_location : next_locations) // Try every possible move. We only add backward edges in this step.
		{
			if (solver->my_heuristic[next_location] <= heuristicBound &&
				...
				(agent % 2 ==0 || isMeetingSatisfied(m, next_location, curr->level + 1))) // the meeting is satisfied TODO Nir this is for agent B only
			{
				...
				for (; child != closed.rend() && ((*child)->level == curr->level + 1); ++child)
				{
					if ((*child)->location == next_location) // If the child node exists
					{
						...
						if (curr->valid) (*child)->valid = true;
						find = true;
						break;
					}
				}
				if (!find) // Else generate a new mdd node
				{
					...
					if (next_location == std::get<0>(task))
	...
	if (goal_node->location == std::get<0>(task))
			goal_node->valid_back = true;
	MDDNode* del = nullptr;
	for (auto parent : goal_node->parents)
	{
		...
		if (goal_node->valid_back) parent->valid_back = true;
	}
	if (del != nullptr)
	...
	for (int t = num_of_levels - 2; t > 0; t--)
	{
		...
		{
			if (node->location == std::get<0>(task))
				node->valid_back = true;
			for (auto parent : node->parents)
			{
				if (node->valid_back) parent->valid_back = true;
	...
	for (auto it : closed)
		if (it->children.empty() && it->level < num_of_levels - 1)
			delete it;
	// Delete nodes that don't enable to pass via the task start (for agent A)
	// TODO Nir this can probably be done during the backward iteration
	if (agent % 2 == 0)
	{
		for (int t = 0; t < num_of_levels; ++t)
		{	
			auto it = levels[t].begin();
			while (it != levels[t].end())
			{
				if (!(*it)->valid && !(*it)->valid_back)
				{
					// TODO Nir use delete node
					// remove it as a parent from its children
...
bool MDD::isMeetingSatisfied(Meeting m, int loc, int t)
{
	if (t == std::get<1>(m))
		return loc == std::get<0>(m);
	return true;
}
...
MDD* MDDTable::getMDD(CBSNode& node, int id, size_t mdd_levels, Task* task)
{
	...
	int task_idx;
	for (Assignment a : *node.assignment)
		if (std::get<1>(a) == id || std::get<2>(a) == id) {
			task_idx = std::get<0>(a);
			break;
		}
	...
	const Instance& instance = search_engines[id]->instance;
	Task task_ = task == nullptr ? instance.getTasks()[task_idx] : *task;
	Meeting m = node.meetings[task_idx];
	mdd->buildMDD(ct, mdd_levels, search_engines[id], id, task_, m);
...
void MDDTable::findSingletons(CBSNode& node, int agent, Path& path, Task* task)
{
	auto mdd = getMDD(node, agent, path.size(), task);
