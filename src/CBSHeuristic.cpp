...
void CBSHeuristic::init()
{
	...
	clearance_cache.resize(num_of_agents / 2); // TODO Nir assuming two agents in a task
}
...
void CBSHeuristic::computeQuickHeuristics(CBSNode& node) // for non-root node
{
	...
	// compute root tie breaking
	if (node.root) node.meetings_tb = computeMeetingsTieBreaker(node);

}

int CBSHeuristic::computeMeetingsTieBreaker(CBSNode& node)
{
	if (root_tb_type == root_tie_breaker_type::NONE) return std::numeric_limits<int>::max();

	int meeting_location, meeting_clearance, 
		node_meeting_radius = root_tb_type == root_tie_breaker_type::MIN ? std::numeric_limits<int>::max() : 0;
	int num_of_tasks = node.meetings_set.size();
	vector<Task> tasks;
	tasks.resize(num_of_tasks);
	for (int i = 0; i < num_of_tasks; ++i)
	{
		tasks[i] = m_meeting_tables[node.assignment_idx][i]->getTask();
	}
	if (node.parent == nullptr) // compute for all meetings in node
	{
		for (int task_id = 0; task_id < num_of_tasks; ++task_id)
		{
			// going over all meetings
			meeting_location = std::get<0>(node.meetings[task_id]);
			meeting_clearance = calcClearance(meeting_location, task_id, tasks);

			// save meeting clearance in cache
			clearance_cache[task_id].insert(std::pair<int, int>(meeting_location, meeting_clearance));

			if (root_tb_type == root_tie_breaker_type::MIN)
				node_meeting_radius = std::min(node_meeting_radius, meeting_clearance);
			else
				node_meeting_radius += meeting_clearance;
		}
	} else // change according to changed meeting
	{
		int new_meeting_idx = node.meetings_set[node.changed_task_idx];
		int new_meeting_location = std::get<0>(node.meetings[node.changed_task_idx]);
		int prev_meeting_idx = new_meeting_idx - 1;
		int prev_meeting_location = std::get<0>(m_meeting_tables[node.assignment_idx][node.changed_task_idx]->getNextMeeting(prev_meeting_idx));
		// previous meeting must be in cache...
		int prev_meeting_clearance = clearance_cache[node.changed_task_idx][prev_meeting_location];
		int new_meeting_clearance;
		if (clearance_cache[node.changed_task_idx].count(new_meeting_location) > 0)
			new_meeting_clearance = clearance_cache[node.changed_task_idx][new_meeting_location];
		else
		{
			new_meeting_clearance = calcClearance(new_meeting_location, node.changed_task_idx, tasks);
			// save meeting clearance in cache
			clearance_cache[node.changed_task_idx].insert(std::pair<int, int>(new_meeting_location, new_meeting_clearance));
		}

		// update the previous value
		node_meeting_radius = node.meetings_tb;
		if (root_tb_type == root_tie_breaker_type::MIN)
		{
			// Get all clearances from cache to re-calculate the minimum
			node_meeting_radius = new_meeting_clearance;
			if (new_meeting_clearance <= node.meetings_tb)
				return node_meeting_radius; // if the new clearnace is lower or equal the previous minimum, than it's the new minimum
			for (int i = 0; i < num_of_tasks; ++i)
			{
				if (i == node.changed_task_idx) continue;
				meeting_clearance = clearance_cache[i][std::get<0>(node.meetings[i])];
				node_meeting_radius = std::min(node_meeting_radius, meeting_clearance);
			}
		}
		else
			node_meeting_radius = node_meeting_radius - prev_meeting_clearance + new_meeting_clearance;
	}
	return node_meeting_radius;
}


int CBSHeuristic::calcClearance(int meeting_location, int task_id, vector<Task>& tasks) const // TODO Nir pass tasks as a reference in many places in the code...
{
	// calculating distance from other tasks' start, goal and meeting locations
	int meeting_radius = std::numeric_limits<int>::max();
	int num_of_tasks = tasks.size();
	int task_start_loc, task_goal_loc, task_meeting_loc, distance_to_start, distance_to_goal;
	for (int other_task_id = 0; other_task_id < num_of_tasks; ++other_task_id){
		if (task_id == other_task_id) continue;

		task_start_loc = std::get<0>(tasks[other_task_id]);
		task_goal_loc = std::get<1>(tasks[other_task_id]);
		// task_meeting_loc = std::get<0>(meetings[other_task_id]); // TODO OFIR: add distance from meetings

		distance_to_start = m_meeting_tables[0][other_task_id]->distance_table[task_start_loc][meeting_location]; // TODO Nir task assignemt index!
		distance_to_goal = m_meeting_tables[0][other_task_id]->distance_table[task_goal_loc][meeting_location]; // TODO Nir task assignemt index!

		meeting_radius = std::min(distance_to_start, distance_to_goal);
	}
	return meeting_radius;
}
...
int CBSHeuristic::solve2Agents(int a1, int a2, const CBSNode& node, bool cardinal)
{
	...
	vector<vector<vector<PathEntry>>> init_paths(1);
	vector<vector<PathEntry>> initial_paths(2);
	initial_paths[0] = *paths[a1];
	initial_paths[1] = *paths[a2];
	init_paths[0] = initial_paths; // TODO Nir what is this function solve2Agents?
	...
	CoCBS cbs(engines, constraints, init_paths, screen);
